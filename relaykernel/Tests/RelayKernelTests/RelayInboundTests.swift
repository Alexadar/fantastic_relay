...
final class RelayInboundTests: XCTestCase {
  ...
  override func setUp() async throws {
    ...
    engine = RelayEngine(config: RelayConfig(listenPort: 0, groupToken: "secret", inboxBound: 1024))
  ...
  func testConnectDirectoryAndPeerRouting() async throws {
    ...
    let byGuid = Dictionary(
      uniqueKeysWithValues: peers.compactMap { p -> (String, String)? in
        guard let g = p["guid"] as? String, let s = p["status"] as? String else { return nil }
        return (g, s)
      })
  ...
  func testKeepaliveEviction() async throws {
    ...
    engine = RelayEngine(
      config: RelayConfig(
        listenPort: 0, groupToken: "secret", evictSecs: 1.5, sweepSecs: 0.5, inboxBound: 1024))
    port = try await engine.start()
